...
beforeEach(async () => {
  ...
  const auth = await api.post('/api/login').send({ username: 'root', password: 'password1' })
  token = auth.body.token
  for (let blog of test_data.manyBlogs) {
    await api.post('/api/blogs').send({ author: blog.author, title: blog.title, url: blog.url, likes: blog.likes }).set('Authorization', 'Bearer ' + token)
  }
})
...
test('deleting a nonexistent blog', async () => {
  const idToDelete = (await api.get('/api/blogs')).body[0].id
  await api.delete(`/api/blogs/${idToDelete}`).set('Authorization', 'Bearer ' + token)
...
test('updating a blog', async () => {
  ...
  await api.put(`/api/blogs/${blogToUpdate.id}`).send(blogToUpdate).set('Authorization', 'Bearer ' + token).expect(200)
  const blogs = (await api.get('/api/blogs')).body
...
test('updating a nonexistent blog', async () => {
  const blogToUpdate = (await api.get('/api/blogs')).body[0]
  blogToUpdate.likes = 100
  await api.delete(`/api/blogs/${blogToUpdate.id}`).set('Authorization', 'Bearer ' + token)
  await api.put(`/api/blogs/${blogToUpdate.id}`).send(blogToUpdate).set('Authorization', 'Bearer ' + token)
  const blogs = (await api.get('/api/blogs')).body
